...
const exportAttendance = async (req, res, next) => {
    try {
        ...
        // 1️⃣ Get all teams from formRegistration
        const regWhere = { formId };
        if (teamCode) regWhere.teamCode = teamCode;
        ...
        if (!registrations.length) {
            return next(new ApiError(404, "No registrations found for this form."));
        }


        // 2️⃣ Get all attendance records
        const attWhere = { formId };
        if (teamCode) attWhere.teamCode = teamCode;
        ...
        // 3️⃣ Merge and keep JSON + XLSX in sync
        const mergedTeams = registrations.map((reg) => {
            const members = reg.value.map((member) => {
                const attRec = attendanceRecords.find(
                    (r) => r.teamCode === reg.teamCode && r.userId === member.user_id
                );
                return {
                    ...
                    isPresent: attRec?.isPresent || false,
                    isPaymentVerified: attRec?.isPaymentVerified || false,
                    markedAtIST: attRec?.markedAt ? formatIST(attRec.markedAt) : "",
                };
            });
        ...
        if (!format || format === "json") {
            return res.status(200).json(mergedTeams);
        }
